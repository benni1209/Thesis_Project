...
```{r}
# MNAR interaction complexity analysis using log-linear models
...
# Quantify interaction using log-linear models
quantify_selection_interaction <- function(selection_prob_table) {
  ...
  # Handle perfect main effects case
...
create_ratio_table <- function(prob_table, scenario_name) {
  ...
  # Create readable scenario names for captions
  readable_names <- list(
...
## Selection Probability Tables for Main Effects Scenarios

The following tables show the selection probabilities for each combination of Y and Z variables. The probabilities are displayed as ratios relative to the lowest probability (scaled to 1:1 as reference).

```{r main-effects-tables, results='asis'}
# Main effects scenarios to display
...
for (scenario in main_effects_scenarios) {
  ...
  # Create and display ratio table
...
- **Pure Main Effects Classic**: Shows a monotonic pattern where higher categories of both Y and Z have progressively higher selection probabilities.

- **Pure Main Effects Non-monotonic**: Demonstrates a non-linear relationship where the middle category of Y has the highest selection probability, while Z shows an irregular pattern.

- **Pure Main Effects Y Only**: Illustrates a scenario where selection depends primarily on variable Y, with Z having minimal influence (constant effect across Z categories).

## Selection Probability Tables for Interaction Effects Scenarios

```{r interaction-effects-tables, results='asis'}
...
# Replace scenario names with readable versions
...
The ratio format makes it easy to see the relative strength of selection bias across different combinations of the target variables, while the summary table quantifies the interaction complexity using statistical measures.